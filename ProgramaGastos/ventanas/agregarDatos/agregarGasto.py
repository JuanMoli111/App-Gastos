...
def build(usuarios, tipos, lista):
    """
...
# #los datos del gasto de los productos que añade a la lista de productos del gasto
    ...
    usuarios = list(map(lambda u: u['nombre'], usuarios))
    ...
    layout= [[sg.Text('Agregar gasto', size=(30,1), font=("Sawasdee", 25), justification= 'center')],
            ...
            [sg.InputCombo(tipos, size=(20, len(tipos) if len(tipos) <= 10 else 10), key = '-tipo-')],
            ...
            [sg.Button('Agregar producto',key='-agregar_producto-'),sg.Listbox(lista,key='-lista_productos-',size=(20,5))],
